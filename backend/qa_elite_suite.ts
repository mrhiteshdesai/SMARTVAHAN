...
const LOG = {
    ...
    success: (msg: string) => console.log(`[PASS] ${msg}`),
    fail: (msg: string) => console.error(`[FAIL] ${msg}`),
    warn: (msg: string) => console.warn(`[WARN] ${msg}`)
};

// Helpers
async function api(method: string, endpoint: string, token: string | null, body?: any) {
...
async function runTest(module: string, name: string, fn: () => Promise<void>) {
...
async function main() {
    ...
    await runTest('Auth', 'Super Admin Login', async () => {
        ...
        if (!superToken) throw new Error('No access token returned');
    });
    ...
    await runTest('UserMgmt', 'Create State (and State Admin)', async () => {
        ...
        if (!res.ok) throw new Error(`Create State failed: ${JSON.stringify(res.data)}`);
    });
    ...
    await runTest('UserMgmt', 'State Admin Login', async () => {
        ...
        if (!res.ok) throw new Error(`State Admin Login failed: ${JSON.stringify(res.data)}`);
    ...
    await runTest('UserMgmt', 'Create OEM (and OEM Admin)', async () => {
        ...
        if (!res.ok) throw new Error(`Create OEM failed: ${JSON.stringify(res.data)}`);
    });
    ...
    await runTest('UserMgmt', 'OEM Admin Login', async () => {
        ...
        if (!res.ok) throw new Error(`OEM Admin Login failed: ${JSON.stringify(res.data)}`);
    ...
    await runTest('UserMgmt', 'Create RTO', async () => {
        ...
        if (!res.ok) throw new Error(`Create RTO failed: ${JSON.stringify(res.data)}`);
    });
    ...
    await runTest('DealerMgmt', 'Create Dealer', async () => {
        ...
        if (!res.ok) throw new Error(`Create Dealer failed: ${JSON.stringify(res.data)}`);
    });
    ...
    await runTest('DealerMgmt', 'Dealer Login', async () => {
        ...
        if (!res.ok) throw new Error(`Dealer Login failed: ${JSON.stringify(res.data)}`);
    ...
    await runTest('QRMgmt', 'Generate QR Batch', async () => {
        ...
        if (!res.ok) throw new Error(`Generate QR failed: ${JSON.stringify(res.data)}`);
        ...
        if (!qr) throw new Error('QR Codes not found in DB after generation (Timeout)');
    ...
    await runTest('CertMgmt', 'Validate QR (Scan)', async () => {
        ...
        if (!res.ok) throw new Error(`Validate QR failed: ${JSON.stringify(res.data)}`);
        if (res.data.data.value !== testData.qrValue) throw new Error('QR Value mismatch');
    });
    ...
    await runTest('CertMgmt', 'Generate Certificate (Happy Path)', async () => {
        ...
        if (!res.ok) throw new Error(`Certificate Generation failed: ${JSON.stringify(res.data)}`);
    });
    ...
    await runTest('CertMgmt', 'Re-use QR (Duplicate Check)', async () => {
        ...
        if (res.ok) throw new Error('Allowed reusing QR code! (Critical Bug)');
    ...
    await runTest('RBAC', 'Dealer cannot create OEM', async () => {
        ...
        if (res.ok) throw new Error('Dealer was able to create OEM!');
...
|----|--------|----------|-------|
${report.bugs.map((b: any) => `| ${b.id} | ${b.module} | ${b.severity} | ${b.error} |`).join('\n')}

## Detailed Log
${report.details.map((d: any) => `- **[${d.status}]** ${d.module}: ${d.name} (${d.error || 'OK'})`).join('\n')}
    `;
